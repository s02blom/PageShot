...
While there are some people who claim selenium is capiable... This can be through "maximing viewport" and or using a "save_full_page_screenshoot()", this seems to be dependant on version (4+) and or which webdriver (geckodriver?) you are using, so does not seem too reliable. There also seems to be other approaches, such as saving [slices of screenshot](https://github.com/petersimeth/selenium-chrome-full-page-screenshot/blob/master/app.py), which is an intresting idea.

## Installation

This project is expected to be run in a devcontainer using the VS Code Dev Containers extension.
